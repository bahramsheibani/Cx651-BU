...
void *new_malloc(size_t size)
{
    ...
    while (curr != NULL)
    {
        if (!curr->in_use && curr->size >= blockSize)
        {

            if (curr->size >= blockSize + sizeof(m_header))
            {
                // Split the block
                m_header *new_block = (m_header *)((char *)(curr + 1) + blockSize);
                new_block->size = curr->size - blockSize - sizeof(m_header);
                new_block->prev = curr;
